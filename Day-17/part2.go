...
func part2(lines []string) int {
	...
	heatLoss := aStar2(grid)

	return heatLoss
}

func aStar2(grid [][]int) int {
	var d = &priorityQueue{}
	...
	for d.Len() > 0 {
		...
		if current.row == len(grid)-1 && current.col == len(grid[0])-1 && current.n >= 4 {
		...
		if current.n < 10 && (current.rowD != 0 || current.colD != 0) {
		...
		if current.n >= 4 || (current.rowD == 0 && current.colD == 0) {
